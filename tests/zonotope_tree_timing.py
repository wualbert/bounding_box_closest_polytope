...
def time_query_zonotope_tree(zonotope_count, dim, num_of_queries):
    ...
    for i in range(zonotope_count):
        ...
        zonotopes.append(zonotope(x, G))
    zt = ZonotopeTree(zonotopes)
    query_points = (np.random.rand(dim, num_of_queries)-0.5)*centroid_range
    start_time = time.time()
...
def query_time_over_zonotope_count():
    ...
    end_zonotope_count = 5
    num = 10

    #time constructions
    box_counts = np.rint(np.logspace(start_zonotope_count,end_zonotope_count,num))
    ...
    for i, bc in enumerate(box_counts):
        ...
        for r in range(repeats):
            print('Repeat ', r)
            runtime[i, r],_root,_bnl = time_query_zonotope_tree(int(bc),dimensions)
