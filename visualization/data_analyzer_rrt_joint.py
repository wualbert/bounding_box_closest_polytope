...
plt.plot(hopper_polytope_counts, hopper_aabb_query_times_max, marker='.', color='b', linewidth=0.5, markersize=7)
plt.plot(pendulum_polytope_counts, pendulum_aabb_query_times_max, marker='.', color='r', linewidth=0.5, markersize=7)
plt.legend(['Hopper', 'Pendulum'])
plt.xlabel('Number of Polytopes')
plt.ylabel('Query Time (s)')
# plt.yscale('log')
# plt.xscale('log')
# plt.ylim(bottom=0, top=0.02)
plt.title('AABB Max Query Time on R3T Datasets')

plt.tight_layout()
plt.savefig('query_time_max.png', dpi=500)
# plt.subplot(212)
